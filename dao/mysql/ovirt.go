...
func CheckOvirtDBConf(o *models.OvirtConf) (err error) {
	var count int64
	result := MysqlBase.Raw(`select count(*) from  pg_info where host = ? and port = ? and dbname = ?`, o.Host, o.Port, o.DBName).Count(&count)
...
func InsertOvirtDBConf(o *models.OvirtConf) (rowAffected int64, err error) {
	...
	result := MysqlBase.Exec(`insert into pg_info(host,port,username,password,dbname,timezone,pgmaxidelconns,pgmaxopenconns,pgconnmaxlifetime,pgprestatement)
...
func ListOvirt() (listovirt []*models.ListOvirtConf, err error) {
	result := MysqlBase.Raw("select aliasname,host,port from pg_info").Scan(&listovirt)
	if result.Error != nil {
...
func GetOvirtConfDetail(aliasname string) (listovirt []*models.GetOvirtConfDetail, err error) {
	result := MysqlBase.Raw("select id,aliasname,host,port,dbname,timezone,pgmaxidelconns,pgmaxopenconns,pgconnmaxlifetime,pgprestatement from pg_info where aliasname = ?", aliasname).Scan(&listovirt)
	if result.Error != nil {
		zap.L().Error("ListOvirt Error!")
		return nil, result.Error
	}
